...
Install `@kong/eslint-plugin-design-tokens`, `eslint`, `eslint-plugin-vue`, and `vue-eslint-parser` as `devDependencies`.

> **Note**: If the host project already installs `@kong/eslint-config-kong-ui` (which depends on `eslint-plugin-vue` and `vue-eslint-parser`), you only need to install `@kong/eslint-plugin-design-tokens` and `eslint` here.
...
```js
'@kong/design-tokens/token-constant-requires-css-var': ['error', { importSources: ['@kong/design-tokens'] }]
```
...
- **`<style>` blocks** — use [`@kong/stylelint-plugin-design-tokens`](../stylelint-plugin-design-tokens/README.md) instead.
