...
Each palette is named `<theme-name>.alias.json` so it corresponds 1:1 with its companion
`themes/<theme-name>.theme.json` (enforced by the naming guard in `themes.spec.mjs`).

```
tokens/alias/color/
  _manifest.json            ← CANONICAL key set. Color groups + step NAMES ONLY (no values, no
                              descriptions). Enforces that every alias exists in every palette.
                              Excluded from all builds. Not a per-theme palette, so it keeps its own name.
  classic-day.alias.json    ← default palette. The main semantic build (SCSS/JS/TS/CSS) resolves against
                              THIS file. Also the `classic-day` theme's palette.
  classic-night.alias.json  ← `classic-night` theme's palette. IDENTICAL values to classic-day — dark
                              mode is a semantic override (see §7), not a different palette.
  konnect-day.alias.json    ← konnect-day (light) palette: standardized steps, day values.
  konnect-night.alias.json  ← konnect-night (dark) palette: standardized steps, night values.
```
...
- **Main build** (`config.mjs`) includes **only** `tokens/alias/color/classic-day.alias.json`. classic-day
  is the source of truth for the default exported output. The other palette files and `_manifest.json` live
...
- **Theme build** (`platforms/themes.mjs` → `aliasIncludesForTheme(name)`): each theme resolves
  against `tokens/alias/color/<name>.alias.json` only.
...
- **Recolor a theme** → edit the `$value` of the relevant step in `tokens/alias/color/<theme>.alias.json` (and
...
1. Create `tokens/alias/color/<theme>.alias.json` with the full `_manifest.json` key set — copy an existing
...
2. Author `themes/<theme>.theme.json` referencing only standardized steps — copy an existing theme of the same
   ...
   (the build and the naming guard in `themes.spec.mjs` fail on any other name); the palette's `.alias.json`
   suffix is likewise required and guarded.
...
- **`classic-day`** — the default. The main build (`config.mjs`) resolves the exported `:root`
  custom-properties against `tokens/alias/color/classic-day.alias.json`, and `themes/classic-day.theme.json`
  is the light theme.
- **`classic-night`** — the dark counterpart. `tokens/alias/color/classic-night.alias.json` is an **exact copy**
...
night, edit the shared palette step in `classic-day.alias.json` **and** `classic-night.alias.json` (they must stay
